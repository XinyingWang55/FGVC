# 第二届“马栏山杯”国际音视频算法大赛-视频补全

## 数据说明
训练集：2194个视频，格式为mp4，视频时长2s~8s。视频大小包括720x1280、1920x1080等。
research1（843个视频，压缩包大小1.98GB），与去年点位跟踪赛道research_1视频集相同（去年数据集里的重复视频现已去重）
research2（1010个视频，压缩包大小2.18GB），与去年点位跟踪赛道research_2视频集相同（去年数据集里的重复视频现已去重）
research3（341个视频，压缩包大小646M），为去年点位跟踪赛道val视频集 + 本赛道新视频

验证集：50个视频，格式为png图像，图像大小为576x1024（我们尽量裁剪掉了角标和字幕区域），帧数120帧以下。每个样本由原视频和任意一种mask（整块挖空、局部水印、随机噪声块、人像）组成。
...
PSNR取值在[0, +∞]，优秀分值范围大约在30~50
SSIM取值在[0, 1]，优秀分值范围大约在0.8~1
```bash
